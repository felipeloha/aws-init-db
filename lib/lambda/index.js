...
module.exports.handler = async function (event) {
    ...
    const connStr = `postgresql://my-stack-database:testPassword@${event.ResourceProperties.host}:5432`;
    await execute(connStr, 'CREATE DATABASE my-stack-database-next;');
    //create users for each DB
    await execute(connStr + '/my-stack-database', 'CREATE EXTENSION pg_trgm;CREATE EXTENSION btree_gin;');
    await execute(connStr + '/my-stack-database-next', 'CREATE EXTENSION pg_trgm;CREATE EXTENSION btree_gin;');
